  ...
  template <class vectorType> class vector {
    ...
    public:
          ...
          enum errorCode {OK = 0, 
                          OUT_OF_RANGE = -1, 
                          BAD_ALLOC = -2, 
                          NOT_FOUND = -3, // not really needed here but shares the same error codes with key-value pairs library because it is convenient: https://github.com/BojanJurca/Cplusplus-key-value-pairs-for-Arduino
    ...
    private:
          ...
          errorCode __changeCapacity__ (int newCapacity, int deleteElementAtPosition = -1, int leaveFreeSlotAtPosition = -1) {
              ...
              for (int i = 0; i < __size__; i++) {
                  ...
                  // check if copying succeded - for example if Arduino Strings are involved it is possible that assignement actually failed without noticing it
                  if (!(newElements [i] == __elements__ [e])) {

                      #ifdef __VECTOR_H_DEBUG__
                          Serial.printf ("Error %i: vector.__changeCapacity__: couldn't copy vector of %i elements\n", (int) BAD_ALLOC, newCapacity);
                      #endif
